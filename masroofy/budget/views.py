...
from .forms import BudgetCycleForm, PinLoginForm, PinSignupForm, AccountUpdateForm, ActiveCycleUpdateForm
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['recent_txs'] = Transaction.objects.filter(cycle=self.active_cycle).order_by('-timestamp')[:5]
        ...
        context['unread_notifications'] = Notification.objects.filter(user=self.request.user, is_read=False)
        return context
...
class SettingsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        active_cycle = BudgetCycle.objects.filter(user=self.request.user, is_active=True).first()
        if active_cycle:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'update_account':
            ...
            active_cycle = BudgetCycle.objects.filter(user=request.user, is_active=True).first()
            if active_cycle:
                ...
                if form.is_valid():
                    ...
                    messages.error(request, "Failed to update cycle. Please check your date constraints.")
...
class TransactionEditView(LoginRequiredMixin, View):
    def get(self, request, pk, *args, **kwargs):
        ...
        return render(request, 'budget/tx_edit_row.html', {'tx': tx, 'categories': Category.choices})
    ...
    def post(self, request, pk, *args, **kwargs):
        try:
            ...
            category = request.POST.get('category')
            note = request.POST.get('note', '')
            tx = TransactionMutationCommand.edit(request.user, pk, amount, category, note)
            return render(request, 'budget/tx_row.html', {'tx': tx})
        except (ValueError, TypeError, InvalidOperation, ValidationError) as e:
