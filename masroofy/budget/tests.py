...
from .models import BudgetCycle, Transaction, User, Notification
...
class FormCoverageTests(TestCase):
    ...
    def test_budget_cycle_form_invalid_dates(self):
        ...
        self.assertTrue(
            any("after the start date" in e for e in form.errors.get("__all__", []))
        )
...
class TemporalRolloverTests(TestCase):
    ...
    def test_retroactive_transaction_does_not_corrupt_daily_tracker(self):
        ...
        tx = Transaction.objects.create(
            cycle=self.cycle, amount=Decimal("500.00"), category="FOOD"
        )
        tx.timestamp = timezone.now() - timezone.timedelta(days=2)
        tx.save()

        TransactionMutationCommand._mutate_balance(
            self.cycle, Decimal("500.00"), past_date
        )
        self.cycle.refresh_from_db()
        self.assertEqual(
            self.cycle.remaining_cycle_balance, initial_balance - Decimal("500.00")
        )
    ...
    def test_validation_boundaries(self):
        ...
        with self.assertRaisesRegex(ValidationError, "strictly after"):
            BudgetCycleService.create_cycle(
                self.user, Decimal("1000"), timezone.localdate(), timezone.localdate()
            )
...
class ExtendedServiceTests(TestCase):
    ...
    def test_edit_transaction(self):
        tx = TransactionMutationCommand.log(
            self.user, Decimal("100.00"), "FOOD", "Lunch"
        )
        edited_tx = TransactionMutationCommand.edit(
            self.user, tx.id, Decimal("150.00"), "FOOD", "Big Lunch"
        )
        self.cycle.refresh_from_db()
    ...
    def test_duplicate_transaction(self):
        tx = TransactionMutationCommand.log(
            self.user, Decimal("200.00"), "BILLS", "Internet"
        )
    ...
    def test_get_cycle_metrics_thresholds(self):
        ...
        self.assertTrue(
            Notification.objects.filter(message__contains="Budget exhausted").exists()
        )
...
class ViewIntegrationTests(TestCase):
    ...
    def test_transaction_delete_htmx(self):
        ...
        response = self.client.post(
            reverse("transaction_delete", args=[tx.id]), HTTP_HX_REQUEST="true"
        )
    ...
    def test_settings_export_and_wipe(self):
        ...
        self.assertEqual(
            response_export["Content-Disposition"],
    ...
    def test_notification_read(self):
        notification = Notification.objects.create(
            user=self.user, message="Budget Warning"
        )
        response = self.client.post(
            reverse("notification_read", args=[notification.id])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_dashboard_invalid_post(self):
        ...
        response = self.client.post(
            reverse("dashboard"), {"amount": "invalid", "category": "FOOD", "note": ""}
        )
        self.assertRedirects(response, reverse("dashboard"))
    ...
    def test_settings_update_password_success(self):
        ...
        self.assertTrue(
            self.client.login(email="test@test.com", password="newpass1234")
        )
    ...
    def test_transaction_edit_get_cancel(self):
        ...
        response = self.client.get(
            reverse("transaction_edit", args=[tx.id]), {"cancel": "true"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_transaction_edit_post_success(self):
        ...
        response = self.client.post(
            ...
            {"amount": "150.00", "category": "TRANSPORT", "note": "Update"},
        )
        self.assertEqual(response.status_code, 200)
        tx.refresh_from_db()
    ...
    def test_history_filter_date(self):
        ...
        tx = TransactionMutationCommand.log(self.user, Decimal("100"), "FOOD")
        response = self.client.get(
            reverse("history"), {"date": str(tx.timestamp.date())}
        )
        self.assertEqual(len(response.context["transactions"]), 1)
    ...
    def test_app_login_redirect_active_cycle(self):
        ...
        self.client.logout()
        response = self.client.post(
            reverse("login"), {"username": "test@test.com", "password": "password123"}
        )
        self.assertRedirects(response, reverse("dashboard"))
